require 'rails_helper'

RSpec.describe JobCategoryResource, type: :resource do
  describe 'serialization' do
    let!(:job_category) { create(:job_category) }

    it 'works' do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(job_category.id)
      expect(data.jsonapi_type).to eq('job_categories')
    end
  end

  describe 'filtering' do
    let!(:job_category1) { create(:job_category) }
    let!(:job_category2) { create(:job_category) }

    context 'by id' do
      before do
        params[:filter] = { id: { eq: job_category2.id } }
      end

      it 'works' do
        render
        expect(d.map(&:id)).to eq([job_category2.id])
      end
    end
  end

  describe 'sorting' do
    describe 'by id' do
      let!(:job_category1) { create(:job_category) }
      let!(:job_category2) { create(:job_category) }

      context 'when ascending' do
        before do
          params[:sort] = 'id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            job_category1.id,
            job_category2.id
          ])
        end
      end

      context 'when descending' do
        before do
          params[:sort] = '-id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            job_category2.id,
            job_category1.id
          ])
        end
      end
    end
  end

  describe 'sideloading' do
    # ... your tests ...
  end
end
